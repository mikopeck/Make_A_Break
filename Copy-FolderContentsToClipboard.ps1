...
.DESCRIPTION
    ...
    For each tracked file, it attempts to identify it as a text file based on the $TextFileExtensions list.
...
.NOTES
    ...
    Corrected the logic for matching file extensions against the $TextFileExtensions list.
...
try {
    ...
    $processedFiles = 0    # Files considered for processing after extension filter
    ...
    if ($allContent.Length -gt 0) {
        ...
        if ($?) { 
            Write-Host "`nSuccessfully copied content of $fileCount text files (out of $processedFiles filtered, total from git: $($gitFiles.Length)) ($([Math]::Round($totalSize/1KB,2)) KB) to clipboard."
        } else {
        ...
        Write-Warning "No text content was gathered from the tracked Git files (either none matched the filter or all were empty/unreadable)."
    }
